...
def main():
    ...
    QUERIES = ["blockchain", "cryptocurrency", "smart contract", "ethereum", "web3"]
    new_count = 0
    for q in QUERIES:
        if len(all_papers) >= TARGET:
            break
        print(f"Query: {q}")
        # Search deep offsets (from 5000 to 10000) to find unseen papers
        for offset in range(5000, 10000, 1000):
