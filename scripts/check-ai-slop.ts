...
export const SLOP_RULES: RuleDefinition[] = [
  ...
  { name: 'not-x-it-is-y', pattern: /\b(?:is|are|was|were)\s+not\s+(?:a |an |just |merely |only |about )?[^.!?]{2,55}\.\s+(?:It|They)\s+(?:is|are|was|were)(?!\s+also\b)/i, description: 'Binary contrast theater (not X. It is Y.)', suggestion: 'State Y. Delete the not-X setup.', category: 'Tier 3 Pattern' },
  { name: 'the part made easy', pattern: /\bThe\s+[a-z][a-z-]{2,30}\s+part\s+(?:made it easy to ignore|did not stay easy to ignore)\b/i, description: 'Parallel part contrast', suggestion: 'State which claim held up in the data.', category: 'Tier 3 Pattern' },
  { name: 'did not prove they did', pattern: /\bdid not prove [^.!?]{3,60}\.\s+They did\b/i, description: 'Did-not-prove / they-did contrast', suggestion: 'Merge into one sentence.', category: 'Tier 3 Pattern' },
  { name: 'signing is not hiring', pattern: /\bSigning is not hiring\b/i, description: 'Binary contrast theater', suggestion: 'Say what was signed without what was funded.', category: 'Tier 3 Pattern' },
  ...
  { name: 'binary contrast not X it is Y', pattern: /\b(?:not merely|are not|aren't|is not a|is not an|is not the|is not one|is not about|is not that|isn't a|isn't an|isn't the) [^.]{5,100}\. (?:It|This|That|They) (?:is|are)\b/i, description: 'Binary contrast theater', suggestion: 'State the positive claim directly.', category: 'Tier 3 Pattern' },
