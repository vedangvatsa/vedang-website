...
- **Model Context Protocol (MCP) Product Server**: Send JSON-RPC 2.0 to \`POST https://veda.ng/.well-known/mcp\` (Tools: \`search_reports\`, \`search_essays\`, \`get_essay\`, \`search_glossary\`, \`get_glossary_term\`).
- **Model Context Protocol (MCP) Docs Server**: Send JSON-RPC 2.0 to \`POST https://veda.ng/.well-known/mcp/docs\` (Tools: \`get_api_documentation\`, \`get_openapi_specification\`, \`get_auth_guide\`, \`search_documentation\`, \`get_course_curriculum\`).
...
- [Veda MCP Manifest](https://veda.ng/.well-known/mcp.json): Multi-server discovery manifest for Product and Docs MCP servers.
- [Veda Agent Plugin](https://veda.ng/plugin.json): Agent Plugins 1.0 manifest and Product/Docs MCP server configuration.
