...
async function fetchPage(query, typeFilter, page) {
  const url = `https://api.openalex.org/works?search=${encodeURIComponent(query)}&filter=from_publication_date:2020-01-01,type:${typeFilter}&per_page=${PER_PAGE}&page=${page}&select=id,title,publication_date,primary_location,authorships,type&sort=cited_by_count:desc&mailto=vatsvedang@gmail.com`;
