...
const STEEM_POSTING_KEY = process.env.STEEM_POSTING_KEY;

const ESSAYS_DIR = path.resolve(REPO_ROOT, 'src/content/essays');
const POSTS_FILE = path.resolve(REPO_ROOT, 'scripts/steem-posts.json');
...
async function publishToSteem(slug: string, title: string, markdown: string) {
  if (!STEEM_USERNAME || !STEEM_POSTING_KEY) {
    throw new Error('STEEM_USERNAME and STEEM_POSTING_KEY are required in .env.local');
  }

  const client = new Client('https://api.steemit.com');
  const key = PrivateKey.fromString(STEEM_POSTING_KEY);
  ...
  if (DRY_RUN) {
    console.log(`[DRY-RUN] Would publish to Steem: ${title}`);
    return `https://steem.blog/${tags[0]}/@${STEEM_USERNAME}/${permlink}`;
  }
...
async function main() {
  if (!fs.existsSync(POSTS_FILE)) {
    console.error('steem-posts.json not found!');
    process.exit(1);
  }
  ...
  if (unposted.length === 0) {
    console.log('✅ All essays have been published to Steem!');
    return;
  }

  const nextPost = unposted[0];
  console.log(`📝 Publishing to Steem: ${nextPost.slug}`);
  ...
  try {
    const url = await publishToSteem(nextPost.slug, essay.title, essay.body);
    console.log(`✅ Published: ${url}`);
    
    if (!DRY_RUN) {
      nextPost.posted = true;
      nextPost.postedAt = new Date().toISOString();
      nextPost.steemUrl = url;
      fs.writeFileSync(POSTS_FILE, JSON.stringify(posts, null, 2));
    }
  } catch (error: any) {
    console.error(`❌ Failed to publish to Steem: ${error.message}`);
    process.exit(1);
  }
}
