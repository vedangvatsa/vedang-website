...
function generateCheckPrompt(check: CheckResult, domain: string): string {
  ...
  p += `\n### Instructions:\n`;
  p += `1. Review our codebase and identify where to apply this.\n`;
  p += `2. Implement the fix cleanly with no syntax, lint, or build errors.\n`;
...
function generateLayerPrompt(layer: LayerScore, domain: string): string {
  ...
  p += `\n### Instructions:\n1. Apply static files and headers to resolve these issues.\n2. Ensure full framework compatibility.\n3. Provide curl commands to verify each fix.`;
  return p;
}
        ...
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
              ...
              <span className="text-[10px] font-medium text-muted-foreground/60 uppercase tracking-wide">{check.layer}</span>
  ...
  return (
    <PageLayout>
      <div className="w-full space-y-8 pb-16">
        ...
        {/* ── Loading ── */}
        {loading && (
          <section className="py-12 rounded-lg border border-border bg-card text-center space-y-4">
          ...
          <section className="w-full space-y-5 animate-in fade-in duration-200">
                ...
                <div className="flex sm:flex-col items-center sm:items-end gap-3 shrink-0">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <Button type="button" variant="outline" size="sm" onClick={() => executeScan(result.url, true)} className="text-xs gap-1.5 h-8 px-3">
                      <IconRefresh className="w-3 h-3 text-muted-foreground" /><span>Re-scan</span>
                    </Button>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2">
              {[
                ...
                { label: 'Robots meta',             active: result.badges.robotsMetaAi },
                { label: 'E-E-A-T signals',         active: result.badges.authorEeat },
                { label: 'No-JS fallback',          active: result.badges.jsRenderingSelfSufficient },
