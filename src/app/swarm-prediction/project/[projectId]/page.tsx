        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className={`transition-all duration-300 overflow-y-auto ${viewMode === 'graph' ? 'hidden' : viewMode === 'report' ? 'w-full' : 'w-1/2'}`}>
            <div className="p-4 sm:p-6 space-y-6">
              <div className="prose max-w-4xl mx-auto" dangerouslySetInnerHTML={{ __html: reportHtml }} />
                ...
                <div ref={chatRef} className="space-y-3 max-h-60 sm:max-h-80 overflow-y-auto mb-3">
                  {messages.map((m, i) => (
                    <div key={i} className={`text-sm ${m.role === 'user' ? 'text-right' : ''}`}>
                      {m.role === 'assistant' ? (
                        <div className="prose prose-sm max-w-4xl mx-auto" dangerouslySetInnerHTML={{ __html: marked(m.content) as string }} />
                      ) : (
