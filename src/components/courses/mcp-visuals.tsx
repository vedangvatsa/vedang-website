      ...
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
          <div className="rounded-[3px] border-2 border-red-200 p-4">
            <div className="text-xs font-bold text-red-500 uppercase tracking-wider mb-2">Before MCP: N×M Problem</div>
          ...
          <div className="rounded-[3px] border-2 border-blue-300 p-4">
            <div className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2">After MCP: N+M Solution</div>
            <p className="text-[11px] text-muted-foreground">5 AI apps + 10 data sources = <strong className="text-blue-600">15 integrations</strong>. Each app implements one client. Each source implements one server.</p>
          </div>
        </div>

        <div className="space-y-1.5">
          {layers.map((l, i) => (
            <div key={l.component} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden" style={{ borderLeftWidth: '3px', borderLeftColor: l.color }}>
          ...
          {transports.map((t) => (
            <div key={t.name} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden">
              ...
              <div className="px-4 py-3 space-y-2">
                <div><span className="text-[10px] font-bold text-muted-foreground/60 uppercase">How it works</span><p className="text-[10px] text-[#37352f]/80 mt-0.5">{t.how}</p></div>
                <div><span className="text-[10px] font-bold text-green-600 uppercase">Best for</span><p className="text-[10px] text-[#37352f]/80 mt-0.5">{t.best}</p></div>
                <div><span className="text-[10px] font-bold text-red-500 uppercase">Limitation</span><p className="text-[10px] text-[#37352f]/80 mt-0.5">{t.limitation}</p></div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {primitives.map((p) => (
            <div key={p.name} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden">
              <div className="px-4 py-2.5 border-b border-[#e3e3e0]" style={{ backgroundColor: p.color + '08' }}>
              ...
              <div className="px-4 py-3">
                <p className="text-[10px] text-[#37352f]/80 mb-2">{p.desc}</p>
                <div className="text-[10px] font-bold text-muted-foreground/60 uppercase mb-1">Use when</div>
                {p.when.map((w) => (
                  <div key={w} className="text-[10px] text-muted-foreground py-0.5 border-b border-[#e3e3e0]/30 last:border-0">{w}</div>
                ))}
              </div>
            </div>
          ))}
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-3">
          {patterns.map((p) => (
            <div key={p.name} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden" style={{ borderLeftWidth: '3px', borderLeftColor: p.color }}>
              ...
              <div className="px-4 py-2.5 grid grid-cols-1 md:grid-cols-3 gap-3 text-[10px]">
                <div>
                  ...
                  {p.tools.map((t) => <p key={t} className="text-[#37352f]/80 mt-0.5 font-mono">{t}</p>)}
                </div>
                <div>
                  <span className="text-muted-foreground/60 uppercase font-semibold">Resources</span>
                  {p.resources.map((r) => <p key={r} className="text-[#37352f]/80 mt-0.5 font-mono">{r}</p>)}
                </div>
                <div>
                  <span className="text-muted-foreground/60 uppercase font-semibold">Key tip</span>
                  <p className="text-[#37352f]/80 mt-0.5">{p.tips}</p>
                </div>
              </div>
            </div>
          ))}
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {categories.map((cat) => (
            <div key={cat.name} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden">
              <div className="px-4 py-2 border-b border-[#e3e3e0]" style={{ backgroundColor: cat.color + '08' }}>
                <span className="text-xs font-bold" style={{ color: cat.color }}>{cat.name}</span>
              </div>
              <div className="px-4 py-2.5">
                {cat.items.map((item) => (
                  <div key={item} className="flex items-center gap-2 py-1 border-b border-[#e3e3e0]/30 last:border-0">
                    <div className="w-3.5 h-3.5 rounded-sm border border-[#e3e3e0] shrink-0" />
                    <span className="text-[11px] text-[#37352f]">{item}</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-2">
          {layers.map(l => (
            <div key={l.name} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden" style={{ borderLeftWidth: '3px', borderLeftColor: l.color }}>
              <div className="px-4 py-2.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {l.items.map(i => (
                    <span key={i} className="text-[10px] px-2 py-0.5 rounded border border-[#e3e3e0] text-muted-foreground">{i}</span>
                  ))}
                </div>
              </div>
            </div>
      ...
      <div className="p-5 md:p-8">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {transports.map(t => (
                <tr key={t.name} className="border-b border-[#e3e3e0]/40">
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-1.5">
          {files.map(f => (
            <div key={f.file} className="rounded-[3px] border border-[#e3e3e0] px-4 py-2.5 flex items-center gap-3" style={{ borderLeftWidth: '3px', borderLeftColor: f.color }}>
        ...
        <div className="mt-4 rounded-[3px] bg-[#f7f6f3] px-4 py-2.5">
          <span className="text-[11px] text-[#37352f]"><strong>Total setup:</strong> Under 65 lines of code. The SDK handles protocol negotiation, message framing, and error serialization.</span>
        </div>
      </div>
    </figure>
  );
      ...
      <div className="p-5 md:p-8">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {clients.map(c => (
                <tr key={c.name} className="border-b border-[#e3e3e0]/40">
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-1.5">
          {patterns.map(p => (
            <div key={p.pattern} className="rounded-[3px] border border-[#e3e3e0] px-4 py-2.5 flex flex-col md:flex-row md:items-center gap-1 md:gap-3">
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {checks.map(c => (
            <div key={c.category} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden">
