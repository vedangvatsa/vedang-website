      ...
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="rounded-[3px] border-2 border-red-200 p-4">
            <div className="text-xs font-bold text-red-500 uppercase tracking-wider mb-3">Traditional Development</div>
          ...
          <div className="rounded-[3px] border-2 border-blue-300 p-4">
            <div className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-3">Vibe Coding</div>
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {steps.map((s) => (
            <div key={s.letter} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden" style={{ borderTopWidth: '3px', borderTopColor: s.color }}>
              <div className="p-3">
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="mt-4 rounded-[3px] bg-[#f7f6f3] px-4 py-2.5">
          <span className="text-[11px] text-[#37352f]"><strong>Key rule:</strong> Never just say "it's not working." Always give the AI the exact error message, the relevant code, and what you expected to happen.</span>
        </div>
      </div>
    </figure>
  );
    ...
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-2">
          {principles.map(p => (
            <div key={p.principle} className="rounded-[3px] border border-[#e3e3e0] p-3">
              <span className="text-xs font-bold text-[#37352f]">{p.principle}</span>
              <p className="text-[10px] text-muted-foreground mt-0.5 mb-1.5">{p.desc}</p>
              <div className="grid grid-cols-2 gap-2">
                <div className="text-[10px] rounded-[3px] bg-red-50 px-2.5 py-1.5"><span className="font-bold text-red-600">Before: </span><span className="text-red-800/70">{p.old}</span></div>
                <div className="text-[10px] rounded-[3px] bg-green-50 px-2.5 py-1.5"><span className="font-bold text-green-600">Now: </span><span className="text-green-800/70">{p.now}</span></div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-1.5">
          {prompts.map(p => (
            <div key={p.category} className="rounded-[3px] border border-[#e3e3e0] px-4 py-2.5 flex items-center gap-3" style={{ borderLeftWidth: '3px', borderLeftColor: p.color }}>
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {gates.map(g => (
            <div key={g.gate} className="rounded-[3px] border border-[#e3e3e0] overflow-hidden">
              <div className="px-3 py-2 border-b border-[#e3e3e0]" style={{ backgroundColor: g.color + '08' }}>
  ...
  return (
    <figure className="not-prose my-8 w-full rounded-[3px] border border-[#e3e3e0] bg-white overflow-hidden">
      <div className="p-5 md:p-8">
        ...
        <div className="space-y-1.5">
          {decisions.map(d => (
            <div key={d.error} className="rounded-[3px] border border-[#e3e3e0] px-4 py-2.5 flex flex-col md:flex-row md:items-center gap-1 md:gap-3" style={{ borderLeftWidth: '3px', borderLeftColor: d.color }}>
