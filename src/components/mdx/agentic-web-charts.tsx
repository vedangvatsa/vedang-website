            ...
            {eras.map((e, i) => {
              ...
              return (
                <div key={e.era} className="flex flex-col items-center text-center">
                  ...
                  <div className="text-[10px] text-[#18181b] font-semibold mt-0.5">{e.era}</div>
            ...
            <div key={d.month} className="grid grid-cols-[90px_1fr_70px] gap-3 items-center">
              ...
              <div className="w-full h-4 bg-[#f7f6f3] rounded-md overflow-hidden">
                <div
                  ...
                  style={{ width: `${(d.downloads / maxD) * 100}%`, backgroundColor: '#18181b', opacity: 0.8 }}
                />
              </div>
              <div className="text-right">
                <span className="text-xs font-bold text-[#18181b]">{d.downloads}M</span>
              ...
              <div className="flex-1 h-6 bg-[#f7f6f3] rounded-md overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: i >= 6 ? '#18181b' : '#37352f',
                    opacity: i >= 6 ? 0.8 : 0.3 + (i * 0.08),
                  }}
                />
              </div>
              <span className={`text-xs font-bold w-12 ${i >= 6 ? 'text-[#18181b]' : 'text-[#37352f]'}`}>{d.label}</span>
            </div>
          ))}
        </div>
...
export function ProtocolStackDiagram() {
  const layers = [
    { name: 'Application Layer', protocols: 'OpenAI Operator, Google Mariner, Genspark Super Agent', desc: 'End-user facing agents that execute tasks', color: 'bg-[#f4f4f5] border-[#18181b]/20' },
    { name: 'Coordination Layer', protocols: 'A2A (Google → Linux Foundation)', desc: 'Agents discover, communicate, and delegate to each other', color: 'bg-[#f4f4f5] border-[#e4e4e7]' },
    { name: 'Tool Layer', protocols: 'MCP (Anthropic → Linux Foundation)', desc: 'Agents connect to databases, APIs, and services', color: 'bg-[#18181b]/15 border-[#e4e4e7]' },
    { name: 'Payment Layer', protocols: 'x402, ACP, MPP', desc: 'Agents transact using stablecoins, cards, and crypto', color: 'bg-[#18181b]/20 border-[#18181b]/50' },
    { name: 'Settlement Layer', protocols: 'Ethereum, Solana, Base', desc: 'Final settlement of value and ownership', color: 'bg-[#18181b]/25 border-[#18181b]/60' },
  ];
          ...
          {layers.map((l) => (
            <div key={l.name} className={`rounded-[3px] border p-4 ${l.color}`}>
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-2">
                ...
                <span className="text-xs font-semibold text-[#18181b] shrink-0 md:text-right">{l.protocols}</span>
              </div>
            </div>
          ))}
        </div>
          ...
          {industries.map((ind, i) => (
            <div key={ind.name}>
              <div className="flex justify-between mb-1">
                ...
                <span className={`text-sm font-bold ${i === 0 ? 'text-[#18181b]' : 'text-[#37352f]'}`}>{ind.pct}%</span>
              </div>
              <div className="w-full h-6 bg-[#f7f6f3] rounded-md overflow-hidden">
                <div
                  className="h-full rounded-md"
                  style={{ width: `${ind.pct}%`, backgroundColor: i === 0 ? '#18181b' : '#37352f', opacity: i === 0 ? 0.8 : 0.3 + (0.1 * (industries.length - i)) }}
                />
              </div>
          ...
          {projections.filter(p => p.value > 0).map((p, i) => (
            <div key={`${p.firm}-${p.note}`}>
              <div className="flex justify-between mb-1">
                ...
                <span className={`text-sm font-bold ${i === 0 ? 'text-[#18181b]' : 'text-[#37352f]'}`}>{p.range}</span>
              </div>
              <div className="w-full h-7 bg-[#f7f6f3] rounded-md overflow-hidden">
                <div
                  className="h-full rounded-md"
                  style={{ width: `${(p.value / max) * 100}%`, backgroundColor: i === 0 ? '#18181b' : '#37352f', opacity: i === 0 ? 0.8 : 0.5 }}
                />
              </div>
              <div className="flex justify-between mt-0.5">
                <span className="text-[10px] text-muted-foreground/70">{p.note}</span>
                {p.cagr && <span className="text-[10px] text-[#18181b] font-semibold">CAGR: {p.cagr}</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {categories.map((cat) => (
            <div key={cat.name} className="rounded-[3px] border border-[#e3e3e0] p-5">
              <div className="text-xs font-bold uppercase tracking-widest text-[#18181b] mb-3">{cat.name}</div>
              <div className="space-y-2">
                {cat.players.map((p) => (
