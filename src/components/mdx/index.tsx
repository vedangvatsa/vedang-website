...
export function Figure({ src, alt, caption, source, sourceUrl, wide = false }: {
  ...
  return (
    <figure className={`not-prose my-8 first:mt-0 last:mb-0 ${wide ? 'max-w-xl mx-auto' : 'max-w-sm mx-auto'}`}>
      <ZoomableImage
        src={src}
        alt={alt}
...
export function Callout({ children, type = 'info', title }: { children: React.ReactNode; type?: 'info' | 'warning' | 'insight'; title?: string }) {
  ...
  return (
    <div className={`not-prose my-6 first:mt-0 last:mb-0 rounded-lg border-l-4 ${styles[type]} p-4 md:p-5`}>
      ...
      <div className="text-sm text-muted-foreground leading-relaxed [&_p]:mb-2 [&_p:last-child]:mb-0 [&_table]:mx-auto [&_table]:max-w-full overflow-x-auto">{children}</div>
    </div>
  );
}
