  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm my-12 w-full font-sans gap-8">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Cold Storage</h3>
        <p className="text-slate-500 mt-2">Keeping crypto keys completely offline for maximum security.</p>
      </div>
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-6 items-center">
        ...
        <div className="text-lg font-bold text-slate-900">{isOn ? "Online (Hot Wallet)" : "Offline (Cold Storage)"}</div>
        <div className={`p-6 rounded-xl border-2 w-full text-center transition-all duration-500 ${isOn ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-emerald-50 border-emerald-200 text-emerald-800'}`}>
          {isOn ? "⚠️ Connected to the internet. Convenient but vulnerable to hacks, malware, and phishing." : "✅ Completely air-gapped. Hardware wallets, paper wallets, or steel backups. Maximum security."}
        </div>
      </div>
    </div>
  );
