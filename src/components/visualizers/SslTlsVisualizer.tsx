...
export function SslTlsVisualizer() {
  ...
  const steps = ["Browser sends \"Hello\" + supported cipher suites.","Server responds with its SSL certificate + chosen cipher.","Browser verifies certificate, both generate session keys.","All data is now encrypted. The padlock 🔒 appears in your URL bar."];

  return (
    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm my-12 w-full font-sans gap-8">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-slate-900 tracking-tight">SSL/TLS</h3>
        <p className="text-slate-500 mt-2">Encrypting data in transit between browser and server.</p>
      </div>
