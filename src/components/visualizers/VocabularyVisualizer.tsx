    ...
    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-lg border border-slate-200 shadow-sm my-12 w-full font-sans gap-8">
      ...
      <div className="w-full max-w-2xl bg-white p-8 rounded-lg border border-slate-200 shadow-sm flex flex-col gap-6">
        <div className="flex gap-3 flex-wrap justify-center">
          {words.map((w, i) => (
            <button key={i} onClick={() => setSelectedWord(i)} className={`px-4 py-2 rounded-lg font-semibold transition-all ${selectedWord === i ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>{w.word}</button>
          ))}
        </div>
