...
export function WebhookVisualizer() {
  ...
  const steps = ["An event occurs (e.g., payment received on Stripe).","Stripe sends an HTTP POST to YOUR server's webhook URL.","Your server receives the payload and processes it (update DB, send email).","No polling needed — your server is notified instantly."];

  return (
    <div className="flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm my-12 w-full font-sans gap-8">
      <div className="text-center">
        <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Webhook</h3>
        <p className="text-slate-500 mt-2">A server-to-server callback triggered by an event.</p>
      </div>
