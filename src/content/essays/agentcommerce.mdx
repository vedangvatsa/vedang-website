...
summary: >-
  When algorithms start shopping on behalf of consumers, traditional e-commerce breaks.
date: '2026-03-24'
author: 'Vedang Vatsa'
keywords: ['agentic commerce', 'AI agents', 'e-commerce', 'x402', 'UCP', 'ACP', 'MPP', 'MCP', 'A2A']
---

Agentic commerce, where AI agents autonomously research, compare, and purchase products on behalf of humans, has moved from concept to live infrastructure in under 18 months. Every major payment network (Visa, Mastercard, PayPal) and AI platform (OpenAI, Google, Anthropic) has shipped agent commerce products. Six competing protocols are in production. Market projections range from $218B (Mordor Intelligence) to $3-5T (McKinsey) by 2030-2031. The binding constraint is trust: 54% of US adults use AI agents, but only 24% trust them to make purchases.

<Figure src="/images/essays/agentcommerce.svg?v=8" alt="Human-to-merchant checkout versus buyer-agent-to-seller-API" width={800} height={400} wide />
...
In traditional e-commerce, you browse, compare, and click. In agentic commerce, you say "get me running shoes under $150, good for flat feet, delivered by Friday" and the agent searches Nike, Amazon, and specialty retailers simultaneously, weighs price against reviews and shipping speed, and completes checkout. You go from *doing the shopping* to *describing what you want*.

You accept a job in a new city and need to relocate your family. Today, that means weeks of juggling real estate listings, moving company quotes, utility transfers, school research, and furniture shopping across dozens of websites. In an agentic world, you describe your budget, commute preferences, and kids' school requirements. The agent scans rental listings, cross-references commute times, identifies neighborhoods with the right schools, gets moving quotes, negotiates rates, schedules utility transfers, and coordinates furniture deliveries to arrive the day you move in. Weeks of browser tabs and phone calls become a few days of conversation.

The shift is structural. Each prior wave of commerce technology (web storefronts in the 2000s, mobile commerce in the 2010s, social commerce in the 2020s) added a new channel. Agentic commerce removes the human from the transaction loop. The agent is a new buyer. The interaction is machine-to-machine, governed by rules written into commerce protocols.

## The convergence of 2026

Three things converged in 2025-2026. Large language models got good enough to parse shopping intent and make reasonable purchase decisions. Google, OpenAI, and Stripe shipped open commerce protocols so agents can talk to any merchant through a standard API. And payment infrastructure caught up: Visa, Mastercard, PayPal, Coinbase, and Stripe each built dedicated products for agent-initiated transactions, including crypto-native protocols like x402 that let machines pay machines. The adoption curve could be faster than prior platform shifts because agents traverse the same digital paths as human shoppers, riding on rails already laid by two decades of e-commerce.


## What has already shipped

Every major platform and payment network has shipped agent commerce products in the past twelve months.

<Timeline>
<TimelineItem date="April 2025" title="Mastercard Agent Pay">
A payment framework where the buyer is a machine. Uses tokenized credentials scoped per-agent and per-merchant.
</TimelineItem>
...
<TimelineItem date="March 2026" title="Stripe + Model launch MPP">
...
<Figure src="/images/essays/agentcommerce/mastercard-agentpay.webp" alt="Mastercard Agent Pay announcement" caption="Mastercard's Agent Pay launch" source="mastercard.com" sourceUrl="https://www.mastercard.com/global/en/news-and-trends/press/2025/april/mastercard-unveils-agent-pay.html" />

</Column>
<Column>

<Figure src="/images/essays/agentcommerce/tobi-tweet.webp" alt="Tobi Lutke tweet on agentic commerce" caption="Shopify CEO on agents as a shopping channel" source="@tobi on X" sourceUrl="https://x.com/tobi/status/1952800271257706676" />

</Column>
</Columns>

<KeyTakeaway>
Traditional payment rails (Visa, Mastercard, PayPal) tokenize existing card networks for agents. Crypto-native protocols (x402, MPP) let agents pay each other in stablecoins over HTTP, bypassing card networks. Both approaches are live and in production. The market has not picked a winner.
</KeyTakeaway>
...
| Protocol | Creator | Type | Payment | Status |
| --- | --- | --- | --- | --- |
| MCP | Anthropic | Tool integration | None (context layer) | Adopted by major AI platforms |
| A2A | Google | Agent coordination | None (negotiation layer) | 150+ organizations |
| AP2 | Google | Payment authorization | Cryptographic mandates | Production |
| ACP | OpenAI + Stripe | In-chat checkout | Stripe card processing | Live in ChatGPT |
| x402 | Coinbase | HTTP-native payments | Stablecoins (USDC) | V2 live, multi-chain |
| MPP | Stripe + Paradigm | Multi-rail payments | Stablecoins, cards, Lightning | Launched Mar 2026 |
...
**Computer use agents** (Anthropic's Claude, OpenAI's Operator) skip structured APIs. These agents control a mouse and keyboard to use existing websites built for humans, which matters where merchants have not built agent-ready APIs. The agent fills out forms, clicks buttons, and completes checkout the same way a person would, faster.
...
McKinsey identified three interaction models ([mckinsey.com](https://www.mckinsey.com/capabilities/mckinsey-digital/our-insights/agentcommerce)).

<Figure src="/images/essays/agentcommerce-models.svg" alt="Three McKinsey interaction models: agent to site, agent to agent, and brokered" source="McKinsey" sourceUrl="https://www.mckinsey.com/capabilities/mckinsey-digital/our-insights/agentcommerce" width={400} height={420} />

In the **agent-to-site** model, your agent talks directly to a merchant's website or API. The merchant may not even know the buyer is an AI. In the **agent-to-agent** model, your shopping agent negotiates with a retailer's commerce agent, both communicating at machine speed through structured protocols. In the **brokered** model, an intermediary agent sits between you and multiple merchants, querying dozens of stores simultaneously.

Each purchase teaches the agent something. After three coffee orders, it knows your preferred roast, your price ceiling, and how fast you go through a bag. By the tenth order, it reorders before you run out. This is predictive commerce: the agent makes decisions based on patterns you never explicitly described.
...
<Figure src="/images/essays/agentcommerce-layers.svg" alt="Five layers of agentic commerce: AI platforms, protocols, payment rails, retail platforms, agent startups" width={400} height={500} />
...
<ResearchPaper
  ...
  abstract="Maps the eight-layer infrastructure stack for autonomous AI agents at production scale, from perception and orchestration to security and deployment."
/>
...
**Commerce Protocols** let agents talk to stores. Five main ones: ACP (OpenAI + Stripe), UCP (Google + Shopify/Walmart/Target), MCP (Anthropic), A2A (Google agent-to-agent), and x402/MPP (Coinbase/Stripe + Model) for machine-to-machine payments. The lack of a single standard is intentional: each protocol optimizes for a different architecture.
...
The startup map shows the same layering. According to the **Agentic Commerce Market Map** by [Artemis Analytics](https://www.artemis.xyz/dashboard/agentcommerce/market-map), there are already over 173 companies operating across 14 distinct categories in the space. 

<Figure src="/images/essays/agentcommerce/artemis-market-map.webp" alt="Agentic Commerce Market Map by Artemis Analytics" caption="Agentic Commerce Market Map" source="Artemis Analytics" sourceUrl="https://www.artemis.xyz/dashboard/agentcommerce/market-map" wide />

The market is currently heavily weighted toward foundational infrastructure rather than consumer-facing apps:
...
With only 7 companies primarily classified as "User Interfaces" (like ChatGPT and Claude), no consumer access point has won yet. The rails are being laid before consumer applications dominate.

## Key numbers

<StatRow>
<Stat value="$218B" label="Retail AI market by 2031" source="Mordor Intelligence" sourceUrl="https://mordorintelligence.com" />
...
These figures are not the same object. Mordor counts a retail-AI market. Morgan Stanley and Bain count US agent-driven e-commerce. McKinsey counts global transaction volume that an agent might touch. They are not one market.

| Firm | Figure | What it counts |
| --- | --- | --- |
| [Mordor Intelligence](https://mordorintelligence.com) | $218B | Retail AI market by 2031 |
| Morgan Stanley (2025) | $190-385B | US e-commerce via agents, 2030 |
| Bain & Company (2025) | $300-500B | US agentic commerce, 2030 |
| [McKinsey](https://www.mckinsey.com/capabilities/mckinsey-digital/our-insights/agentcommerce) | $3-5T | Global transaction volume, 2030 |

36% of consumers say they are open to AI making purchases for them ([SWAP Commerce](https://swap-commerce.com)), and 84% consider one-click checkout important ([WPFactory](https://wpfactory.com)). McKinsey projects agent-intermediated transaction volume could reach $3-5 trillion globally by 2030. During Cyber Week 2025, one in five orders involved an AI agent. Morgan Stanley estimates US e-commerce spending through agentic shoppers could reach $190-385 billion by 2030, representing 10-20% of total US e-commerce. Bain projects the US agentic commerce market at $300-500 billion by the same year.

<StatRow>
<Stat value="$3-5T" label="Projected transaction volume by 2030" source="McKinsey" sourceUrl="https://www.mckinsey.com/capabilities/mckinsey-digital/our-insights/agentcommerce" />
<Stat value="29.3%" label="Compound annual growth rate" source="Mordor Intelligence" sourceUrl="https://mordorintelligence.com" />
...
Brand loyalty weakens. If your agent switches you to a cheaper toothpaste that works as well, you probably will not notice. Predictive reordering kicks in: your agent knows you buy coffee every three weeks and handles it before you think about it.

<Figure src="/images/essays/agentcommerce/meme-marketer.webp" alt="Every e-commerce marketer in 2026" caption="The marketer's new reality" />

<Figure src="/images/essays/agentcommerce-value.svg" alt="What loses value and what gains value when the buyer is an algorithm" width={400} height={440} />

Product pages built for human eyes matter less than structured data built for agent consumption: schema.org markup, real-time inventory feeds, machine-readable specs. Traditional SEO gives way to what Commercetools calls "Answer Engine Optimization" and "LLM Optimization" ([commercetools.com](https://commercetools.com)). Display ads and retargeting lose relevance when the buyer is an algorithm. The traditional idea of visiting Amazon for shopping or Expedia for travel gives way to a single concierge that handles shopping and travel when your intent first surfaces.
...
Agentic commerce restructures how products get discovered, how buying decisions are made, and how customer relationships form.

**Product discovery and engagement.** Companies need to build agents that understand customer intent and proactively suggest products, bundles, or alternatives. Product catalogs need semantic and behavioral metadata embedded at the data layer, more than keywords on a webpage. When an agent asks "what running shoes do you have under $150 with good arch support in size 11?", the response needs to come from structured data.
...
**Core commerce platforms.** Existing e-commerce infrastructure was built around a human clicking through pages. Agents need structured transaction endpoints where they can query inventory, check pricing, and complete checkout in a single API call. Dynamic pricing, real-time inventory awareness, and machine-readable return policies become requirements.

**Payments and fraud.** Authentication moves from stopping bots to enabling the right agents to transact for the right customers. Know Your Customer (KYC) standards must extend to Know Your Agent (KYA): verifying users and the agents acting on their behalf. Tokenization, dispute resolution, and settlement systems need to carry richer context about agent identity within transaction metadata.
...
A stack built for a human clicking pages will miss the customer that never loads a page.
...
Most agent interactions are still free, and providers have not developed clear revenue strategies.


## The trust gap and fraud problem

There is a 30-point gap between agent usage and purchase trust: 54% of US adults use AI agents, but only 24% trust them to buy things. Top consumer concerns: payment security (32%), privacy (26%), agent errors (18%), loss of control (17%). According to a July 2025 [Kearney US Consumer Study](https://www.kearney.com/documents/d/asset-library-291362522/agentcommerce-retail-disruption-pdf), roughly 30% of the market consists of "privacy-conscious skeptics" who demand absolute data control and assurances that agent mistakes will be financially corrected before they will adopt agentic shopping.

<Figure src="/images/essays/agentcommerce-trust.svg" alt="54 percent of US adults use AI agents, 24 percent trust them to buy, 13 percent have completed an AI purchase" width={400} height={280} />

<Figure src="/images/essays/agentcommerce-concerns.svg" alt="Top consumer concerns: payment security 32 percent, privacy 26 percent, agent errors 18 percent, loss of control 17 percent" source="Kearney US Consumer Study, July 2025" sourceUrl="https://www.kearney.com/documents/d/asset-library-291362522/agentcommerce-retail-disruption-pdf" width={400} height={300} />
...
AI agents cannot form binding contracts in most jurisdictions. [Visa's analysis](https://www.visa.co.uk/content/dam/VCOM/regional/ve/unitedkingdom/PDF/agentic-payments.pdf) notes that under GDPR and the ePrivacy Directive, agents capturing personal data via web scraping or mimicking tracking footprint cookies require explicit legal bases. Some laws also mandate direct user acceptance of terms and conditions, which leaves autonomous purchasing agents in a legal bind when they act without active confirmation.
</Callout>

</Column>
<Column>

<Figure src="/images/essays/agentcommerce/meme-agent.webp" alt="Is this a great deal?" caption="The agent trust dilemma" />
...
Autonomous agents are decision-makers operating at scale. A single faulty prompt or model update can trigger cascading failures: incorrectly booked flights, overordered inventory, purchases made without proper consent. When agents are interconnected across multiple systems and protocols, minor errors can compound exponentially. The question is whether an agent makes good decisions today, and whether the system can contain and recover from failures. Can agents backtrack? Can purchases be reversed automatically?
...
AI agents operate on data, and data sovereignty is increasingly a geopolitical issue. Countries like India and France enforce data localization. Europe is pushing for AI sovereignty. If an agent processes EU citizen data through a US-based API, compliance is unclear. If it is trained on global data but acts locally, the legal status is ambiguous. OpenAI has already launched a "for countries" model offering localized infrastructure and regulatory flexibility, which shows how hard it is to scale agent platforms without breaching national data protection and consumer protection laws.

Trust is contextual. What feels intuitive in one market can be unacceptable in another. For agents to earn trust, they need transparent decision trails, fail-safes, reversibility, and boundaries the user can set.
...
**Google.** Open standard, owned distribution. UCP is open. Search and Gemini are not. Same playbook as Android.

**OpenAI.** Discovery and intent. About 800 million weekly active users. Moving from owning checkout to owning where the ask starts.

**Coinbase.** Machine-to-machine pay. If agents settle in USDC over x402, Coinbase takes a cut no matter which model wins the chat.

**Shopify.** Infrastructure on every AI surface, owned by none of them. Checkout Kit, Catalog, Universal Cart.

**Stripe and Paradigm.** One protocol across stablecoins, cards, and Lightning. Backed by Anthropic, OpenAI, Mastercard, and Shopify.

**Amazon.** Purchase history, reviews, and logistics behind Rufus and Alexa+. Agents can still read third-party reviews. That is the hole.


<KeyTakeaway>
Whoever controls the agent decides what gets bought. Visa and Mastercard are lending their brand to machines they did not train. Until a court names who pays for a bad purchase, signed mandates and reversibility matter more than a new checkout. The 30-point gap between use and purchase trust is the queue. Close it with a living consent trail.
</KeyTakeaway>
