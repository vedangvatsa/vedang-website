...
SaaS companies sell seats. AI agents do not need seats. When agents perform the work that seats were priced to enable, the revenue model collapses. The $400 billion SaaS market is being disrupted by a buyer who does not use the product the way it was priced.
...
Together, MCP and A2A are building the plumbing that allows agents to treat every SaaS product as an API endpoint rather than a destination. The SaaS vendor becomes middleware that agents route through rather than an interface that humans navigate.

## What survives

The vendors that last will look different from the ones that sell dashboards today.

**Agent-native platforms.** Instead of adding AI features to existing dashboards, they rebuild as infrastructure that agents consume. The API is primary, the dashboard is optional, and the documentation is written for agent consumption.

**The data layer.** Companies that control proprietary datasets (Salesforce with CRM data, Snowflake with warehouse data, Bloomberg with financial data) retain value because agents need the data regardless of the interface. The interface is the disposable layer.

**Value pricing.** Stable models tie revenue to measurable business outcomes: $0.99 per resolved ticket, $1.00 per qualified lead, $0.10 per CRM action. Those prices move with the customer's ROI. Per-seat pricing remains only where a human still has to own the decision.

<KeyTakeaway>
If the renewal forecast still assumes seat expansion, it is already in conflict with the agent product. Salesforce cut its own support staff from 9,000 to 5,000 while shipping Agentforce. Intercom only bills when Fin closes the ticket. Price the outcome, or keep the seat only where a human still has to own the decision.
</KeyTakeaway>
