...
- The discovery that scaling neural networks on text produces reasoning was not predicted
...
If the most important developments are, by definition, the ones no one sees coming, then the timeline that matters most is the one no current forecast captures. This is not a reason to abandon forecasting. It is a reason to treat forecasts as calibrated uncertainty rather than point predictions.
