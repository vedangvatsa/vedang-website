...
The transition is from a field dependent on corporate data access to one built on diverse, resilient data sources: open protocols, legally mandated access, donated personal data, controlled experiments, and computational simulation. Each source has limitations. The combination provides the methodological strength that the Twitter-dependent era lacked.
