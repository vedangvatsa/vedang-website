...
Every major AI lab CEO predicts AGI within 3-8 years. The three largest regulatory powers, the EU, the US, and China, are building incompatible governance frameworks for current AI, and none of them address the structural challenges that AGI will introduce. The governance gap is institutional.

<Figure src="/images/essays/governance.svg?v=7" alt="Infographic" wide />
...
China's approach is the most pragmatic. It requires that every deployed algorithm be registered, inspectable, and traceable rather than banning categories of AI. The system assumes that the government should always be able to see what AI systems are doing and who built them.
...
This means that AGI, potentially the most powerful technology ever created, will likely be developed and initially controlled by a small number of private companies, most of them headquartered in two countries. The governance question is who decides how AGI is governed, and on whose behalf.
...
The most durable model spreads the capability: more than one lab, more than one country, something inspectable. If the path is downloadable weights, Mistral's Apache cards are the clean version. Llama is [a community file](/licenses) with a 700 million user veto. Meta can still say no. An international inspectorate is the other path. Whether either happens before one or two labs own the capability is the race.

<KeyTakeaway>
Lab CEOs place AGI in a three-to-eight-year window. No framework in force has an AGI clause, and no inspectorate can walk into a training run. The IAEA was built before nuclear weapons were widely proliferated. The lab equivalent has to exist while the models are still slower than the hearing. After that, oversight is a meeting trying to govern a system that can outrun the meeting.
</KeyTakeaway>
