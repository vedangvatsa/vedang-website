...
In 2024, Gmail began drafting replies before users opened the email. GitHub Copilot started writing functions before programmers finished describing them. Spotify generated playlists calibrated to the listener's circadian rhythm. In each case, the software acted not on a command but on a prediction of what the user was about to want. The gap between human intent and machine execution, which has defined every generation of computing since the punch card, is collapsing.
...
**Power asymmetry.** An anticipatory AI system that models your intent, preferences, and behavioral patterns possesses detailed knowledge of your cognitive profile. The entity (corporation, government, individual) that controls this system has utilize over your behavior that is qualitatively different from traditional forms of influence.
