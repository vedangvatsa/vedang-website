...
In 2016, AlphaGo played move 37 in game 2 against Lee Sedol. Commentators called it a mistake. It won the game. The network had not walked a rule tree out loud. It had ranked a pattern from millions of board positions.

<Figure src="/images/essays/intuition.svg?v=8" alt="Infographic" wide />

## Two Modes of Human Thought

Daniel Kahneman split that kind of snap judgment from slow arithmetic in *Thinking, Fast and Slow* (2011). **System 1** is the glance: a chess player seeing a move, a firefighter leaving a room before the floor goes. **System 2** is 347 times 28 in your head. AI labs spent decades on System 2: expert systems, theorem provers, planners. Those tools died on messy rooms and incomplete rules.

<PullQuote>
Expert intuition is pattern recognition trained by experience, operating on representations too complex to articulate but too useful to ignore.
</PullQuote>

The current generation of AI, particularly large neural networks, may represent the first meaningful attempt at System 1 replication. They process information in a manner that is parallel, associative, and pattern-driven rather than sequential and rule-based.

## What expert intuition is
...
This recognition-primed decision model treats intuition as extensive domain experience compressed into rapid pattern matching. The expert has seen so many variations of a problem that the underlying structure of a new instance is immediately recognizable, even when surface features differ.

<Callout type="insight" title="The 10,000-hour threshold">
K. Anders Ericsson's research on deliberate practice (1993) suggests that expert-level intuition typically requires approximately 10,000 hours of domain-specific experience. This provides an empirical benchmark: intuition is the result of extensive training that encodes complex patterns into fast, automatic retrieval systems. The question for artificial systems is whether analogous pattern encoding can be achieved computationally, and if so, whether it constitutes "intuition" in any meaningful sense.
</Callout>
...
**Protein structure prediction.** AlphaFold (2020) predicts three-dimensional protein structures from amino acid sequences with accuracy approaching experimental methods. The system recognizes patterns in the relationship between sequences and structures, learned from approximately 170,000 known structures, rather than reasoning through biochemical principles step by step. The output resembles the intuitive judgment of an experienced structural biologist, arrived at through a different process.

**Medical image interpretation.** Deep learning systems for radiology, pathology, and dermatology match or exceed specialist-level performance in detecting specific conditions (diabetic retinopathy, certain cancers, skin lesions). These systems recognize visual patterns associated with disease rather than applying diagnostic criteria sequentially, in a manner that is structurally analogous to how experienced clinicians develop a "clinical eye" through years of case exposure.
...
The connection between intuition and knowledge structure can be modeled through network theory. Human knowledge is organized as a dense, interconnected semantic network where concepts are nodes and relationships are edges with varying strengths.
...
<PullQuote>
The path to artificial intuition may require building systems that can combine fast pattern recognition with slow deliberation, and that can calibrate their confidence in each mode's outputs.
</PullQuote>

## Implications for Problem-Solving

If artificial intuition is achievable, even in the narrow functional sense described here, it shows up first in a few kinds of work.
...
**Decision support under uncertainty.** In domains where decisions must be made faster than System 2 allows (emergency medicine, financial trading, military operations), artificial intuition may become an extra System 1 for human decision-makers. The system recognizes patterns in real-time data that human System 1 has not been trained to detect, and surfaces them for human evaluation.
...
<KeyTakeaway>
Chain-of-thought prompting and test-time compute already pair fast pattern recognition with a slow check, which is the part of Kahneman's dual-process model that transfers. The training-set advantage is largest across domains, because a human expert is bounded by one career of cases. Until a system can tell when its own match is unreliable, a person still has to run that check. Point the stack at datasets too large for a person to hold, then keep the override.
</KeyTakeaway>
