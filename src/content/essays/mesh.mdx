...
In 1937 [Ronald Coase](https://en.wikipedia.org/wiki/Ronald_Coase) asked why firms exist. His answer: finding a partner, writing a contract, and enforcing it costs more than doing the work inside one shop. Drop those costs and the shop should shrink. Drop them near zero with smart contracts, agents, and protocols, and the shop unravels into a mesh: people and bots rewiring around a protocol instead of a manager.

<Figure src="/images/essays/mesh.svg?v=7" alt="Infographic" wide />
...
The answer: markets carry friction, and that friction determines the boundary between what happens inside organizations and what happens between them.
...
<Figure src="/images/essays/charts/coasean-transaction-friction.png" alt="Infographic comparing traditional vs protocol transaction cost reductions: search (99.9% reduction), contract writing (~100%), dispute resolution (95%)" wide />
...
DeFi is the first large-scale implementation of mesh economics, and the volumes are large.
...
<Figure src="/images/essays/charts/dao-treasury-concentration.png" alt="Donut chart showing DAO Treasury Concentration: Uniswap 23.5% ($3.2B), Optimism 20.6% ($2.8B), Arbitrum 15.4% ($2.1B), Mantle 11% ($1.5B), ENS 7.4% ($1.0B), others 22.1% ($3.0B)" wide />
...
AI agents on Uniswap, Aave, MCP, and A2A are a new kind of participant: software that finds a pool, signs a contract, and settles without a person in the loop.
...
The mesh economy promises to eliminate intermediaries. But the protocols that enable the mesh are themselves intermediaries, albeit transparent, automated ones. Uniswap charges a 0.3% fee on every trade. Aave takes a spread on every loan. The fee is lower than a traditional intermediary's, but it is not zero. And the governance of these protocols is concentrated among early investors and large token holders, replicating the power structures the mesh was designed to replace. The mesh compresses hierarchy into protocol parameters and governance votes.
</Callout>
...
What Coase did not predict was that the agents doing the coordinating would themselves be software. The mesh is increasingly a network of AI agents as well as humans operating within protocol-defined boundaries, executing transactions at speeds and volumes no human organization could match.
...
<KeyTakeaway>
Uniswap is a protocol with token holders, not a firm with no bosses. Read who holds the tokens before you call it a mesh. Keep people on the jobs that cannot be verified in a block.
</KeyTakeaway>
