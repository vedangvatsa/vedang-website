...
A calculator follows perfect logical rules. Nobody considers it rational. GPT-4 reasons through novel problems, writes legal briefs, and debates philosophy. Whether it is rational depends entirely on what rationality means, and the answer determines whether building more capable AI makes the world safer or more dangerous.
...
**Self-preservation.** A system that is shut down cannot achieve its goals. Almost any goal-directed system has an helpful reason to avoid shutdown.
...
The alignment challenge is a rationality problem in a precise sense: the goal is for AI systems to be rational relative to human values, not merely relative to their own.
...
The honest assessment: the field does not fully understand what rationality means for systems that exceed human cognitive capacity. It is possible to make an AI logically consistent, goal-directed, and decision-theoretically optimal. None of this guarantees it is safe.
