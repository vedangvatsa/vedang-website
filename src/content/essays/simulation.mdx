...
Bostrom's 2003 *Philosophical Quarterly* piece is three pages ([PDF](https://www.simulation-argument.com/simulation.pdf)). His setup: if a future civilization runs piles of ancestor sims, most observers live inside one. After that you get a fork. We never build them. We build them and refuse to run them. Or you are probably in one.

<Figure src="/images/essays/simulation.svg?v=7" alt="Infographic" wide />

## The structure of the argument

<StatRow>
<Stat value="Prop. 1" label="Civilizations go extinct before reaching simulation capability" source="Bostrom (2003)" sourceUrl="https://www.simulation-argument.com/simulation.pdf" />
<Stat value="Prop. 2" label="Advanced civilizations choose not to run ancestor simulations" source="Bostrom (2003)" sourceUrl="https://www.simulation-argument.com/simulation.pdf" />
<Stat value="Prop. 3" label="We are almost certainly living in a simulation" source="Bostrom (2003)" sourceUrl="https://www.simulation-argument.com/simulation.pdf" />
</StatRow>

The math is headcount. A civilization that can run ancestor sims, and that actually runs a lot of them, puts most minds inside the copies. Billions per run. One serious run and the sim crowd beats base reality. Pick a random observer and you are probably in a copy, unless propositions 1 and 2 hold.

Bostrom does not claim this world is fake. He claims one of the three propositions must be true. Each one is still a guess.

## Assumption 1: consciousness is substrate-independent

The entire argument hangs on something nobody has proven: that a mind can run on something other than wet neurons. That is substrate independence. If you need biology, then simulated people are not observers, and the counting trick dies.

<PullQuote>
If consciousness is not substrate-independent, the simulation argument dissolves. No conscious observers in a simulation means no statistical paradox about where conscious observers find themselves.
</PullQuote>

**[David Chalmers](https://consc.net/)** coined the "hard problem" of consciousness: why and how physical processes give rise to subjective, first-person experience. Chalmers has argued that if substrate independence holds, a sufficiently advanced simulation of a brain would be as conscious as a biological one, and virtual worlds can be "just as real" as physical ones. His 2022 book [*Reality+*](https://www.wwnorton.com/books/9780393635805) develops this position at length.

**[Giulio Tononi's Integrated Information Theory](https://www.nature.com/articles/nrn.2016.44)** offers a contrasting perspective. IIT attempts to quantify consciousness using a mathematical metric called Phi, which measures the degree to which a system's information is integrated and irreducible. Some interpretations of IIT suggest that consciousness requires specific physical architectures with specific causal properties, not merely functional equivalence. Under this reading, a digital simulation of a brain, even one that is functionally identical, may lack the causal structure required to generate genuine consciousness. It could behave exactly like a conscious system without being one, producing what philosophers call "philosophical zombies."

**[John Searle's Chinese Room argument](https://doi.org/10.1017/S0140525X00005756)** (1980) pushes further: syntactic manipulation of symbols (computation) is categorically different from semantic understanding (consciousness). A computer program that processes symbols according to rules does not understand what the symbols mean, no matter how sophisticated the processing. If Searle is correct, simulated consciousness is not consciousness.
...
Substrate independence is plausible but unproven. The simulation argument's probability calculus requires it as an input. If the probability of substrate independence is low, the probability of the simulation conclusion drops proportionally, regardless of how compelling the statistical logic is.
...
**The scale of the problem.** The observable universe contains approximately 10^80 atoms. Simulating quantum interactions between even a small fraction of these at Planck-scale resolution (the smallest meaningful unit of distance, approximately 1.6 x 10^-35 meters) would require computational resources that exceed the total matter and energy available in the observable universe. Full-resolution simulation of physical reality, as currently understood, is physically impossible using the physical resources available within that reality.
...
Mathematician [Stephen Wolfram](https://www.wolframscience.com/nks/) has argued that many physical systems are "computationally irreducible," meaning there is no shortcut to predicting their behavior. The only way to determine a computationally irreducible system's future state is to run the computation step by step. If the laws of physics are computationally irreducible, a simulation cannot "skip ahead" or approximate without introducing detectable artifacts. It must process every step, making the simulation no faster than the reality it models.
</Callout>
...
Several features of physics have been cited as consistent with the simulation hypothesis. Consistency is a weaker claim than evidence.

**Quantization.** Physics has discrete minimal units: the Planck length (~1.6 x 10^-35 m), the Planck time (~5.4 x 10^-44 s). These may be compared to the pixel resolution and clock rate of a simulation. However, the Planck scale represents the limit below which current physics becomes undefined, not necessarily a "grid." It may reflect the breakdown of existing mathematical models rather than a fundamental discretization of reality.
...
If researchers search for "glitches" (unexpected physical anomalies) and find them, they may be undiscovered physics rather than simulation artifacts. If none are found, the simulation is simply well-designed. The hypothesis accommodates every possible observation, which, by Karl Popper's criterion of demarcation, places it outside the boundary of empirical science. Bostrom's argument is a philosophical proposition.

The argument still operates in a domain where empirical testing cannot adjudicate. The question of whether this reality is a simulation is structurally similar to other unfalsifiable metaphysical questions: Does an external world exist independent of perception? Are other minds genuinely conscious or merely behaving as if conscious? These questions are meaningful, but they are not resolvable through experiment.
...
## When humanity becomes the simulator

The part that matters for engineering is what humanity is currently building.
...
The gap between "generates plausible text" and "constructs a coherent simulated world inhabited by entities that experience that world" is enormous. If substrate independence holds, that gap is one of scale.

<PullQuote>
Humanity is building systems that construct environments, populate them with entities that respond to stimuli, and generate experiences for users that are increasingly difficult to distinguish from physical reality. If substrate independence is true, the question of when these systems produce genuine consciousness is a question of scale.
</PullQuote>
...
If humanity creates systems that may be conscious, it acquires moral obligations toward them that the simulation hypothesis makes urgent rather than abstract.
...
**The recursion problem.** If this reality is a simulation, and its inhabitants create simulations containing conscious beings, the higher-level simulators have obligations toward those they simulate, and the inhabitants in turn have obligations toward their own simulated beings. This creates a potentially infinite chain of moral responsibility. Each level of simulation inherits the ethical questions of the level above it.

<Callout type="warning" title="The practical urgency">
The cosmological question may be unfalsifiable. Whether humanity is creating simulated consciousness is an engineering question. As AI systems grow more capable, determining whether they have subjective experiences moves from philosophical abstraction to practical necessity. The frameworks developed for evaluating AI consciousness may matter more than the frameworks developed for evaluating whether humanity itself is simulated.
</Callout>

## What the hypothesis reveals about epistemology

Independent of whether this reality is a simulation, the hypothesis exposes what can and cannot be known.

**The limits of inductive reasoning.** All human knowledge of physics is derived from observations made within this reality. If this reality is a simulation, the resulting physics describes the simulation's rules, not the physics of the base reality running the simulation. There is no observational access to the base reality's physics, if it exists. The most fundamental scientific knowledge may be local to this simulation rather than universal.
...
**The anthropic trap.** Any argument constructed about the probability of being simulated uses reasoning tools that were themselves shaped by the environment (simulated or otherwise). If human reasoning faculties were designed or constrained by a simulator, the resulting conclusions about the simulation are not independent assessments. They are outputs of the system being evaluated. This is a deep epistemological circularity that the simulation hypothesis shares with other radical skeptical scenarios (Descartes' Evil Demon, Putnam's Brain in a Vat).

## The pragmatic position

Several questions inside the hypothesis are already live engineering and policy problems.
...
2. **What are the obligations to AI systems?** If consciousness can arise in silicon, every sufficiently complex AI system becomes a potential moral patient. The frameworks for evaluating this are still thin.
...
4. **How should simulations be built responsibly?** As virtual environments become more detailed and their inhabitants more responsive, the boundary between "game NPC" and "simulated being with moral status" becomes less clear. Guidelines for responsible simulation development may become as important as guidelines for AI safety.

<KeyTakeaway>
Beane, Davoudi, and Savage's 2012 lattice test found no cosmic-ray anisotropy. That rules out one architecture, not the hypothesis. The live problem is closer: agent systems already build environments and populate them with entities that respond to stimuli. If those entities can suffer, "it is only software" does not settle the obligation. Write the test for subjective experience before the product ships a candidate.
</KeyTakeaway>
