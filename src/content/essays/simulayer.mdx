...
Boeing tests factory changes on a digital twin before it cuts metal. Waymo logs tens of billions of miles in simulation. Drug candidates get screened in software before a clinical trial. Climate models run thousands of futures. The physical object is often the second draft.

<Figure src="/images/essays/simulayer.svg?v=7" alt="Infographic" wide />
...
NASA kept computer models of spacecraft in the early 2000s so ground crews could diagnose a ship they could not touch. Apollo 13's rescue was that idea with slide rules: simulate the crippled stack, then radio the procedure. Boeing now does the same for factory flow before it cuts metal. MarketsandMarkets puts the digital-twin market at about $10 billion in 2023 and **$73 billion by 2030**. Those are vendor TAM prints. The live cases below are Boeing, Siemens, and a heart model.

<SimulationDomainChart />

### Boeing: Manufacturing Without Building

[Boeing](https://www.boeing.com/) keeps digital twins of how a plane gets built. Engineers try the assembly order, catch clashes, and fix the flow before parts exist. They put the save at more than **$1 billion** in avoided rework and delay (Boeing/industry estimates).

The 777X was the first Boeing designed entirely in software. Every part, every step, every tool path ran in sim first. When metal started moving, thousands of clashes that used to show up on the floor were already gone. Those used to cost about $100,000-500,000 each.
...
[Waymo](https://waymo.com/) has accumulated over **20 billion simulated miles**, orders of magnitude more than its physical driving experience. As of December 2025, Waymo had surpassed **170 million cumulative rider-only miles** on public roads. The gap between simulated and physical experience is the strategy.
...
Simulated miles can also be targeted. If a rare scenario (e.g., a bicycle approaching from a blind corner at dusk) is identified as a weakness, millions of variations of that specific scenario can be generated and tested overnight. Physical testing cannot achieve this density of targeted practice.
...
The impact on drug discovery is direct. Understanding a protein's 3D structure is a prerequisite for designing molecules that interact with it. The cost comparison:
...
But the gap between best current simulation (available to Boeing, Google DeepMind, and national weather services) and what is available to a small company or developing nation remains significant. A climate model running at 1km resolution requires computing resources that cost millions of dollars per simulation run. A full autonomous driving simulation stack costs tens of millions to develop and maintain. Access is still expensive first. The cost falls later.

## The Convergence: Simulation + AI Agents

The next frontier is simulation by AI agents: autonomous systems that can design experiments, run simulations, interpret results, and iterate without human intervention at each step.
...
The implication is that the simulation layer is becoming an autonomous decision-making environment where AI agents operate independently, with humans setting objectives and reviewing outputs rather than directing each step.
...
<KeyTakeaway>
Run the decision in the twin before you spend the metal. Waymo's 20 billion sim miles are why the 170 million physical miles were survivable. The firm that simulates first decides first.
</KeyTakeaway>
