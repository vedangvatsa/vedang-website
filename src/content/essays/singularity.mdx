...
I am not picking the year. [That clock](/asi) is a separate argument. So is [why a detailed forecast of this is already false](/paradox). Copilot finishing your function is [a different essay](/instinct). Twenty-one Neuralink patients is [another](/liminal).

<Figure src="/images/essays/singularity.svg?v=7" alt="Infographic" wide />

## Defining the Singularity

Powerful is not the claim. The claim is a system that can design a better version of itself. [I.J. Good](https://doi.org/10.1016/S0065-2458(08)60418-0) called that an "intelligence explosion" in 1965: each generation builds the next, and the gains outrun any ministry, lab board, or statute that tries to keep up.
...
<PullQuote>
Alignment is not a bugfix. It is trying to write down what people want, when people do not agree and cannot say it cleanly, then handing that note to something that will take it literally.
</PullQuote>
...
The biggest jumps in this field were not on any forecast. Keep the calendar. Treat the surprise as [the paradox](/paradox).

## What Happens After

If it happens and we are still here, the interesting cases are not utopia or extinction. They are the ones in the middle that look aligned in the demo and are not. There are many ways to miss the spec and few ways to hit it. "Almost aligned" is harder to catch than a paperclipper.
