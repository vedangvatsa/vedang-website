...
Track how AI research expanded from early neural network studies to the current LLM and agentic era.
...
<StatRow>
  ...
  <Stat label="Top Bigram" value="Neural Network (11,949)" />
  <Stat label="Fastest Rising Vector" value="Agentic (449x)" />
</StatRow>

The bigram data reveals five distinct strategic research pillars driving current engineering efforts:

1. **Production-Grade ML:** "Machine learning" (9,765) and "deep learning" (7,030) confirm the transition to production deployment. "Object detection" (4,205) remains a massive subfield for industrial applications, tracking logistics, manufacturing defects, and autonomous navigation.
2. **The LLM Infrastructure Wave:** "Large language" (4,391) shows that LLM research has become one of the dominant bigrams. Enterprises are actively researching how to deploy models with enterprise-grade RAG architectures rather than relying purely on bulk internet scraping.
3. **Autonomous Navigation & Robotics:** "Autonomous driving" (3,602) and "reinforcement learning" (4,597) form a critical cluster. Deep reinforcement learning is the primary driver for advancements in physical robotics, moving beyond digital text generation into real-world kinetic operations.
4. **Clinical Diagnostics:** "Medical image segmentation" (454 as a trigram) and "speech recognition" (3,577) validate healthcare as the highest-stakes and most heavily funded deployment domain.
