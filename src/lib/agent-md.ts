  ...
  switch (pathname) {
    ...
    case '/pricing':
      return [
        ...
        '',
        '**Rate limits**: Standard keyless bucket, 60 requests/minute per IP. Headers on all API responses: RateLimit-Limit: 60, RateLimit-Remaining, RateLimit-Reset.',
        '',
        '## Enterprise / Custom (Contact)',
        '',
        'For high-volume ingestion, SLA guarantees, or custom integrations: https://veda.ng/contact (response within 2 business days). Options: dedicated rate-limit tier, bulk JSONL data exports, custom MCP tool exposure.',
        '',
        '## Machine Endpoints',
        '',
        '- `POST /api/v1/scan`: AI readiness audit (JSON score + checks)',
        '- `GET /api/v1/essays`, `/api/v1/glossary`: catalog indexes (JSON)',
        '- `POST /api/v1/reports/search`: academic paper search (JSON)',
        '- `GET /openapi.json`: OpenAPI 3.1 specification',
        '- `POST /.well-known/mcp`, `POST /.well-known/mcp/docs`: MCP servers (JSON-RPC 2.0)',
        '- `GET /llms.txt`, `/llms-full.txt`: LLM catalog and digest',
        '',
        '## Usage Policy',
        '',
        '- Reading, research, and machine consumption: allowed.',
        '- Training: subject to robots.txt AI-bot directives.',
        '- Commercial redistribution without written consent: not permitted.',
        '',
